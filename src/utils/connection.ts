...
async function getSalesforceOrgInfo(): Promise<SalesforceCLIResponse> {
  try {
    const stdout = await execSilent("sf org display --json");

    console.error("[Salesforce CLI] Command completed successfully");

    let response: SalesforceCLIResponse;
    try {
      response = JSON.parse(stdout);
    } catch (parseErr) {
      throw new Error(`Failed to parse Salesforce CLI JSON output: ${stdout}`);
    }

    if (response.status !== 0) {
      throw new Error(
        `Salesforce CLI command failed with status: ${response.status}`
      );
    }

    if (!response.result?.accessToken || !response.result?.instanceUrl) {
      throw new Error(`Salesforce CLI did not return required credentials`);
    }
    ...
    if (error instanceof Error) {
      if (
        error.message.includes("sf: command not found") ||
        error.message.includes("'sf' is not recognized")
      ) {
        throw new Error("Salesforce CLI (sf) is not installed or not in PATH");
      }
    }
    throw new Error(
      `Failed to get Salesforce org info: ${
...
export async function createSalesforceConnection(config?: ConnectionConfig) {
  ...
  try {
    if (connectionType === ConnectionType.OAuth_2_0_Client_Credentials) {
      ...
      console.error(
        "Connecting to Salesforce using OAuth 2.0 Client Credentials Flow"
      );
      ...
      console.error(
        "Connecting to Salesforce using Username/Password authentication"
      );
